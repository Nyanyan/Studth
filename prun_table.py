...
def table_phase0():
    ...
    que = deque([[solved1, solved2, 0, i] for i in range(24)])
    ...
    while que:
        ...
        if cnt % 10000 == 0:
            print(cnt, len(que))
        idx1, idx2, cost, direction = que.popleft()
        cost += 1
        for twist_idx, twist in enumerate(candidate[0]):
            n_cost = cost
            if not can_rotate[direction][twist // 6]:
                n_cost += 1
                n_dirs = [move_dir(direction, 12), move_dir(direction, 13)]
            else:
                n_dirs = [direction]
            n_idx1 = trans_ep[idx1][twist_idx]
            n_idx2 = trans[idx2][twist_idx]
            if table[n_idx1][n_idx2] > n_cost:
                table[n_idx1][n_idx2] = n_cost
                for n_direction in n_dirs:
                    que.append([n_idx1, n_idx2, n_cost, n_direction])
